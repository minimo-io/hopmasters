...
class SharedServices {
  static Future<bool> isLoggedIn() async {
    ...
    return Sharedprefs.getString("login_details") != '{}' &&
            Sharedprefs.getString("login_details") != null
        ? true
        : false;
  }
  ...
  static Future<LoginResponse?> loginDetails() async {
    ...
    return Sharedprefs.getString("login_details") != '{}' &&
        ...
        ? LoginResponse.fromJson(
            jsonDecode(Sharedprefs.getString("login_details")!))
        : null;
  }
  ...
  static Future<OrderData?> lastShippingDetails() async {
    ...
    return Sharedprefs.getString("last_shipping_details") != '{}' &&
            Sharedprefs.getString("last_shipping_details") != null
        //? OrderData.fromJson(jsonDecode(Sharedprefs.getString("last_shipping_details")!))
        ? OrderData.fromJson(
            jsonDecode(Sharedprefs.getString("last_shipping_details")!))
        : null;
  }

  static Future<OrderData?> removeLastShippingDetails() async {
  ...
  static Future<void> setLastShippingDetails(OrderData? lastOrderData) async {
    final Sharedprefs = await SharedPreferences.getInstance();
    Sharedprefs.setString("last_shipping_details",
        lastOrderData != null ? jsonEncode(lastOrderData.toJson()) : "{}");
  }
  ...
  static Future<void> savePreferences(
      String preferenceType, UnmodifiableListView<Pref> prefList) async {
    final Sharedprefs = await SharedPreferences.getInstance();

    if (preferenceType == "beer_types")
      Sharedprefs.setString("beer_types", jsonEncode(prefList));
    if (preferenceType == "news_types")
      Sharedprefs.setString("news_types", jsonEncode(prefList));
  }
  ...
  static Future<List<dynamic>?> getPreferences(String preferenceType) async {
    final Sharedprefs = await SharedPreferences.getInstance();
    if (preferenceType == "beer_types") {
      return Sharedprefs.getString("beer_types") != '{}' &&
              Sharedprefs.getString("beer_types") != null
          ? jsonDecode(Sharedprefs.getString("beer_types")!)
          : null;
    }
    if (preferenceType == "news_types") {
      return Sharedprefs.getString("news_types") != '{}' &&
              Sharedprefs.getString("news_types") != null
  ...
  static Future<void> populateProvider(
      BuildContext context, String preferenceType) async {
